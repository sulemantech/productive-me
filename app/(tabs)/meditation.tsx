...
/**
 * MeditationReflectionScreen: A screen for guided meditation and reflection sessions.
 */
const MeditationScreen = ({ onBack }) => {
  // Timer state management
  const [initialSeconds, setInitialSeconds] = useState(15 * 60); // Default to 15 minutes for reflection
  ...
  const [postSessionReflection, setPostSessionReflection] = useState(false); // Renamed for clarity
  ...
  useEffect(() => {
    ...
    if (isRunning && secondsRemaining > 0) {
      ...
      Alert.alert(
        'Session Complete',
        'Meditation session complete! Take a moment to reflect.',
        [
  ...
  // Function to set the timer to a default value (now 15 minutes)
  ...
  return (
    <ScrollView
      style={meditationStyles.scrollView}
      contentContainerStyle={meditationStyles.contentContainer}
    >
      <View style={meditationStyles.card}>
        {/* Header with back button and title */}
        <View style={meditationStyles.headerContainer}>
          <TouchableOpacity onPress={onBack} style={meditationStyles.backButton}>
            <Text style={meditationStyles.backButtonText}>{'<'}</Text>
          </TouchableOpacity>
          <Text style={meditationStyles.title}>Meditation & Reflection</Text>
        </View>

        {/* Timer Display Section */}
        <View style={meditationStyles.timerContainer}>
          ...
          <View style={meditationStyles.timerTextWrapper}>
            <Text style={meditationStyles.timerText}>{formatTime(secondsRemaining)}</Text>
            <View style={meditationStyles.timerModeButtons}>
              <TouchableOpacity onPress={handleSetDefaultTimer}>
                <Text style={meditationStyles.timerModeText}>{Math.floor(initialSeconds / 60)} min</Text>
              </TouchableOpacity>
              <Text style={meditationStyles.timerModeSeparator}> / </Text>
              <TouchableOpacity onPress={() => setShowCustomTimerModal(true)}>
                <Text style={meditationStyles.timerModeText}>Custom</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        {/* Motivational Text */}
        <Text style={meditationStyles.motivationalText}>"The quieter you become, the more you are able to hear."</Text>

        {/* Start / Pause Button */}
        <TouchableOpacity
          style={[meditationStyles.sessionButton, isRunning ? meditationStyles.pauseButton : meditationStyles.startButton]}
          onPress={handleSessionControl}
        >
          <Text style={meditationStyles.sessionButtonText}>
            {isRunning ? 'Pause' : 'Start Session'}
          </Text>
        </TouchableOpacity>

        {/* Session Goal Input */}
        <View style={meditationStyles.inputContainer}>
          <Text style={meditationStyles.label}>Set your intention or focus</Text>
          <TextInput
            style={meditationStyles.input}
            placeholder="e.g., Focus on gratitude"
        ...
        {/* Optional Reminders */}
        <View style={meditationStyles.checkboxContainer}>
          <View style={meditationStyles.checkboxWrapper}>
            <Switch
              ...
              value={dhikrReminder}
            />
            <Text style={meditationStyles.checkboxLabel}>Remind me to do dhikr after session</Text>
          </View>
          <View style={meditationStyles.checkboxWrapper}>
            <Switch
              trackColor={{ false: '#767577', true: '#06B6D4' }}
              thumbColor={postSessionReflection ? '#F4F3F4' : '#F4F3F4'}
              onValueChange={setPostSessionReflection}
              value={postSessionReflection}
            />
            <Text style={meditationStyles.checkboxLabel}>Show post-session reflection prompt</Text>
          </View>
        </View>

        {/* Session History / Stats */}
        <View style={meditationStyles.statsContainer}>
          <Text style={meditationStyles.statsTitle}>Reflection Stats</Text>
          <View style={meditationStyles.statsList}>
            <Text style={meditationStyles.statsItem}>🧘‍♀️ 4 sessions completed this week</Text>
            <Text style={meditationStyles.statsItem}>🔥 Best streak: 3 days in a row</Text>
            <Text style={meditationStyles.statsItem}>😌 Most mindful time: 7am - 8am</Text>
          </View>
        </View>
      </View>
      ...
      >
        <View style={meditationStyles.modalOverlay}>
          <View style={meditationStyles.modalContent}>
            <Text style={meditationStyles.modalTitle}>Set Custom Time</Text>
            <TextInput
              style={meditationStyles.modalInput}
            ...
            />
            <View style={meditationStyles.modalButtonRow}>
              <TouchableOpacity
                style={meditationStyles.modalButton}
                onPress={() => setShowCustomTimerModal(false)}
              >
                <Text style={meditationStyles.modalButtonText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[meditationStyles.modalButton, meditationStyles.modalSaveButton]}
                onPress={handleCustomTimerSave}
              >
                <Text style={meditationStyles.modalButtonText}>Save</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
